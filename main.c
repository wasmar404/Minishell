...
/*   Updated: 2024/10/24 13:13:30 by wasmar           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main(int ac,char **av,char **envp) 
{
    ...
    (void)av;
    (void)input;
    t_env *head = env_to_linked_list(envp);
    (void)head;
    char  **my_envp =  env_to_array(head);
    (void)my_envp;
    char *prompt = BOLD_CYAN "sw_shell> " RESET;
    while (1)
    {
...
void main_helper(char *input,char **envp)
{
    ...
    char **splitted_input = token_split(input);
    // (void)splitted_input;
    // (void)envp;
     t_token *head = input_to_linked_list(splitted_input,envp);
     t_env *head_env = env_to_linked_list(envp);
...
{   
    if(head->next && head->prev && head->next->type == PIPE && head->prev->type == PIPE)
        return (3);    
    else if(head->next && head->prev && head->next->type == WORD && head->prev->type == PIPE)
    {
...
void run_command(t_token *head,char **current_command,char **envp,t_env *my_envp,int *pipefd);
void complicated_execute(t_env *my_envp, t_token *head,char *envp[])
{
    ...
    while(head != NULL)
    {
        ...
        {
            if(head -> next && (head -> next -> type == WORD || head->next->type == DIRECTORY))
            {
    ...
    if(forkid == 0)
    {
        ...
        else
        {
            dup2(pipefd[0],0);
             close(pipefd[0]);
             close(pipefd[1]);
             char *path = find_path_of_cmd(head->token,envp); 
