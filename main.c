...
void	external_commands(t_token *head, t_env *my_envp, t_exe *exe,
...
void	run_command_helper(t_token *head, t_env **my_envp, t_shell *shell,
		t_exe *exe)
{
	char	**current_command;
	...
	if (path_exists(exe->envp) == 1)
	{
		if (head->built_in_or_not == true)
			...
			external_commands(head, (*my_envp), exe, current_command, shell);
	}
	else
	{
		shell->exit_code = 127;
