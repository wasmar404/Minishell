...
void								init_shell_struct(t_shell *shell,
										char **envp);
void								init_mallo_struct(t_malloc **mallo,
										t_shell *shell);
...
void								add_shell_level(t_env **my_envp,
...
void								run_built_ins_helper(t_token *head,
										t_env **my_envp, t_shell *exitcode);
bool								find_var_name_first(t_env **my_envp,
										char *var_name);
void								run_built_ins_unset(t_token *head,
										t_shell *shell, t_token **tokenn);
void								run_built_ins(t_token *head, int flag,
										t_exe *exe, t_shell *shell);
// main_helper_5.c
...
void								external_commands(t_token *head, t_exe *exe,
										...
										t_shell *exitcode);
// main_helper_6.c
void								main_heredoc_helper(t_token *head,
										t_shell *shell, int *i);
void								main_heredoc(t_token *head, t_shell *shell);
void								delete_temp_files(t_shell *shell);

// main
int									main_helper_helper(char *input,
										t_shell *shell, char ***splitted_input,
										t_malloc *mallo);
