...
/*   Updated: 2024/10/23 08:25:19 by wasmar           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    int len = count_nodes(head);  // Number of nodes in the list
    char **my_envp = malloc((len + 1) * sizeof(char *));  // Allocate for 'len' strings + NULL terminator
    int i = 0;
    ...
    while (head != NULL)
    {
        int str_len = ft_strlen(head->all);  // Length of current string
        my_envp[i] = malloc((str_len + 1) * sizeof(char));  // Allocate space for the string + '\0'
