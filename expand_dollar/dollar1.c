...
/*   Updated: 2025/02/13 15:21:25 by schaaban         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "dollar.h"

void	if_expanded(t_variables *var, int *i, t_token **head)
{
	char	*temp;

	temp = NULL;
	temp = ft_strjoin(var->tertiary, var->primary);
	free(var->tertiary);
	var->tertiary = temp;
	free_2_variables(var->primary, var->secondary);
	expand_variable(head, var->tertiary, var->i);
